window.onload = function() {
	...
	function all(joueur, evenement) {
		...
		if (already_use.length == 0 && al == 0) {
			al = 1;
			$("#dammage").html("<h1 class='dam1'>joueur "+joueur+" a toi de jouer</h1>");
		...
		while (i < vaisseau.length) {
			...
			if (evenement == 1) {
				...
				if (wicht == joueur && vaisseau[i].getAttribute('name') == use_now) {
					vaisseau[i].addEventListener("dblclick", do_watch);
				}
				etape = 1;
			}
			if (evenement == 0) {
				...
				if (wicht == joueur && already_use.indexOf(vaisseau[i].getAttribute('name')) == -1) {
					vaisseau[i].addEventListener("dblclick", do_pp);
				}
				etape = 0;
			}
			if (evenement == 2) {
	...
	function do_pp(e) {
		...
		var i = 0;
		while (i < vaisseau.length) {
			vaisseau[i].removeEventListener("dblclick", do_pp);
			i++;
		}
		var id = e.target.id;
		...
		$.post("get_bonus.php", {name:name}, function(data) {
			...
			document.getElementById('avance').onclick = function() {
			...
			document.getElementById('bouclier').onclick = function() {
			...
			document.getElementById('tire').onclick = function() {
			...
			var ok = 0;
			document.getElementById('vie').onclick = function() {
			...
			document.getElementById('valider').onclick = function() {
				...
				$.post("remp_bonus.php", {ok:ok, name:name, vie:vie, bouclier:bouclier, pos_plus:pos_plus}, function(data) {
					$("#carac").hide();
					all(joue, 1);
				});
			}
	...
	function attack(e) {
		if (e.shiftKey) {
			...
			$.post("attack.php", {name:name}, function(data) {
				...
				document.getElementById('no_tire').onclick = function() {
	...
	function do_watch(e) {
		...
		var i = 0;
		while (i < vaisseau.length) {
			vaisseau[i].removeEventListener("dblclick", do_watch);
			i++;
		}
		id = e.target.id;
		name = document.getElementById(id).getAttribute('name');
		$.post("deplacement.php", {name:name}, function(data) {
			...
			while (a < 4) {
				if (a == 0) {
					deb = dep_pos_left(deu, deb, max_dep, 0);
				} else if (a == 2) {
					deb = dep_pos_bottom(deu, deb, max_dep, 0);
				} else if (a == 1) {
					deb = dep_pos_right(prem, deb, max_dep, 0);
				} else if (a == 3) {
					dep_pos_top(prem, deb, max_dep, 0);
				}
				a++;
			}
			...
			document.getElementById('no_deplace').onclick = function() {
				var deplace = document.querySelectorAll(".go");
				var z = 0;
				while (z < deplace.length) {
